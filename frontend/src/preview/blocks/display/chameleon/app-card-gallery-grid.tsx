...
import { Code2, MessageSquare, Workflow, Globe, MoreVertical, Plus } from 'lucide-react';
...
const KIND: Record<Kind, { label: string; Icon: typeof Code2; tileBg: string; tileFg: string; badgeBg: string; badgeFg: string }> = {
...
const CARDS: { kind: Kind; name: string; key: string; desc: string; status?: { label: string; bg: string; fg: string }; embedded?: boolean }[] = [
  { kind: 'code', name: '智能客服 Agent', key: 'app-svc-bot', desc: '基于工单知识库的多轮客服，含工具调用与人工审批转接。', status: { label: '已发布 v3', bg: '#ecfdf5', fg: '#047857' } },
  { kind: 'chatflow', name: '商品文案助手', key: 'app-copywriter', desc: '上传商品图，自动生成多平台营销文案。', status: { label: '草稿', bg: '#f5f4ee', fg: '#78716c' }, embedded: true },
  { kind: 'workflow', name: '财报抽取流程', key: 'flow-finance', desc: 'PDF 文档抽取 → 分类 → 结构化入库的编排流程。', status: { label: '已发布 v1', bg: '#ecfdf5', fg: '#047857' } },
    ...
    <PreviewFrame bg="#fafaf7" padded={false}>
      <div style={{ padding: 24, fontFamily: sans }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(248px, 1fr))', gap: 12 }}>
          ...
          {/* 虚线新建卡 */}
          <button style={{ height: 148, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 12, border: '1px dashed #d6d3d1', background: 'rgba(255,255,255,0.6)', color: '#78716c', cursor: 'pointer', fontFamily: sans }}>
            <div style={{ display: 'flex', height: 36, width: 36, alignItems: 'center', justifyContent: 'center', borderRadius: '50%', background: '#f5f4ee' }}>
          ...
          {CARDS.map((c, i) => (
            <Card key={c.key} card={c} hovered={i === 0} />
          ))}
...
function Card({ card, hovered }: { card: typeof CARDS[number]; hovered?: boolean }) {
  const m = KIND[card.kind];
  const Icon = m.Icon;
  return (
    <div
      style={{
        position: 'relative', display: 'flex', height: 148, flexDirection: 'column',
        borderRadius: 12, border: `1px solid ${hovered ? '#bfdbfe' : 'rgba(231,229,224,0.8)'}`,
        background: '#fff', padding: 16, textAlign: 'left', cursor: 'pointer',
        boxShadow: hovered ? '0 4px 6px rgb(0 0 0 / 7%), 0 2px 4px rgb(0 0 0 / 4%)' : '0 1px 2px rgb(0 0 0 / 5%)',
      }}
    >
      {/* hover 顶部渐变高光线 */}
      <span style={{
        pointerEvents: 'none', position: 'absolute', left: 16, right: 16, top: 0, height: 2,
        borderRadius: 9999, background: 'linear-gradient(to right, transparent, #3b82f6, transparent)',
        opacity: hovered ? 1 : 0,
      }} />

      {/* 三点菜单（hover 浮现） */}
      <div style={{ position: 'absolute', right: 8, top: 8, opacity: hovered ? 1 : 0 }}>
        <button style={{ display: 'flex', height: 28, width: 28, alignItems: 'center', justifyContent: 'center', borderRadius: 6, color: '#a8a29e', background: 'transparent', border: 'none', cursor: 'pointer' }}>
          <MoreVertical size={16} color="#a8a29e" strokeWidth={2} />
        </button>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          ...
          borderRadius: 8, background: m.tileBg, color: m.tileFg,
