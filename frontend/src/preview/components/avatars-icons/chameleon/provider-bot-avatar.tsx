 ...
 * (A) ProviderAvatar 按 provider code 给品牌色+短标(8 key)   (B) violet→blue 渐变 Bot 头像
 * 源码：core/components/common/provider-avatar.tsx + playground message-thread / playground-page
 *
 * 配色取自源码 BRAND map 的 Tailwind 阶；new-api / oneapi 用 --color-primary-*
 * （默认 blue 主题，随 data-primary 切换），此处取默认值。
 */
...
const BRANDS = [
  ...
  // new-api / oneapi 同一组 primary-* 配色（默认 blue）；两个独立 key
  { code: 'new-api', label: '⇄', bg: '#eff6ff', fg: '#2563eb', ring: '#dbeafe', primary: true },
  { code: 'oneapi', label: '⇄', bg: '#eff6ff', fg: '#2563eb', ring: '#dbeafe', primary: true },
  ...
  { code: '? (unknown)', label: '?', bg: '#f5f5f4', fg: '#78716c', ring: '#e7e5e4' },
];
  ...
  return (
    <PreviewFrame bg="#fafaf7">
      <div style={{ maxWidth: 720, margin: '0 auto', fontFamily: 'Inter, system-ui, sans-serif', color: '#1c1917' }}>
        ...
        {/* A · 品牌单字方头像 sm (32px) — 8 个 brand key */}
        <Section title="A · ProviderAvatar · sm (h-8 w-8 / 32px) · 8 key">
          {BRANDS.map(b => (
            <div key={b.code} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: 9.5, fontFamily: 'JetBrains Mono, monospace', color: b.primary ? '#2563eb' : '#a8a29e' }}>{b.code}</span>
            </div>
          ))}
        </Section>
        ...
        <Section title="真实用法">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <div style={{ maxWidth: 360, borderRadius: 10, background: '#fffefb', border: '1px solid #e7e5e4', padding: '8px 12px', fontSize: 12.5, lineHeight: 1.5, color: '#44403c' }}>
                您好，我是客服助手。请问有什么可以帮您？
              </div>
            </div>
            {/* 对比列头 */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, borderBottom: '1px solid rgba(231,229,228,0.7)', background: 'rgba(244,243,238,0.3)', padding: '8px 12px', borderRadius: 6 }}>
