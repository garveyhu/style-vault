...
import { Activity, Bot, Coins, Database, Sparkles, TrendingDown, TrendingUp, Users, Zap } from 'lucide-react';
...
const TILES: Array<{ label: string; value: string; hint: string; delta: number; inverse?: boolean; icon: IconCmp; tone: string }> = [
  ...
  return (
    <PreviewFrame bg="#fafaf7">
      <div style={{ maxWidth: 800, margin: '0 auto', fontFamily: 'Inter, system-ui, sans-serif', color: '#1c1917' }}>
        ...
        {/* StatTile 大卡 */}
        <div style={{ fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#78716c', marginBottom: 12 }}>StatTile · delta + tone chip</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
          {TILES.map(t => {
            const hasDelta = t.delta !== 0;
            const up = t.delta > 0;
            const down = t.delta < 0;
            const good = t.inverse ? down : up;
            const deltaColor = !hasDelta ? '#a8a29e' : good ? '#059669' : '#dc2626';
            const tone = TONE[t.tone];
            const Icon = t.icon;
            return (
              <div key={t.label} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', borderRadius: 12, border: '1px solid #e7e5e0', background: '#ffffff', padding: 20 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 12, color: '#78716c' }}>{t.label}</div>
                  <div style={{ marginTop: 8, fontFamily: 'monospace', fontSize: 24, letterSpacing: '-0.02em', color: '#1c1917', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.value}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {MINIS.map(m => {
            ...
            const Icon = m.icon;
            return (
              <div key={m.label} style={{ display: 'flex', alignItems: 'center', gap: 12, borderRadius: 12, border: '1px solid #e7e5e0', background: '#ffffff', padding: '12px 16px' }}>
        ...
        <div style={{ marginTop: 20, fontSize: 11, color: '#a8a29e', lineHeight: 1.7 }}>
          deltaInverse（成本/错误率）下降才显绿 · 大数字 font-mono tnum · chip 浅底深字 · StatBar 极淡竖发丝分隔
        </div>
      </div>
    </PreviewFrame>
  );
