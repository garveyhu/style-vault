import { PreviewFrame } from '../../../_layout';

export default function EmeraldSwitch() {
  return (
    <PreviewFrame bg="#fafaf7">
      <div style={{ maxWidth: 600, margin: '0 auto', fontFamily: 'Inter, system-ui, sans-serif', color: '#1c1917' }}>
        <div style={{ fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#78716c' }}>COMPONENT · TOGGLE</div>
        <h1 style={{ fontSize: 28, fontWeight: 600, letterSpacing: '-0.02em', margin: '8px 0 32px' }}>Emerald Switch</h1>

        <Row label="ON · running 任务">
          <Switch on />
        </Row>
        <Row label="OFF · stopped 任务">
          <Switch />
        </Row>
        <Row label="disabled">
          <Switch on disabled />
        </Row>

        <div style={{ fontSize: 12, color: '#a8a29e', marginTop: 24 }}>
          h-5 w-9 (20×36) · thumb h-4 w-4 · translate-x-4 切换 · checked emerald-500 / unchecked stone-300
        </div>
      </div>
    </PreviewFrame>
  );
}

function Switch({ on, disabled }: { on?: boolean; disabled?: boolean }) {
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', width: 36, height: 20, borderRadius: 9999,
      background: on ? '#10b981' : '#d6d3d1', padding: 2, opacity: disabled ? 0.5 : 1, cursor: disabled ? 'not-allowed' : 'pointer',
      transition: 'background 200ms',
    }}>
      <span style={{ width: 16, height: 16, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 2px rgb(0 0 0 / 10%)', transform: on ? 'translateX(16px)' : 'translateX(0)', transition: 'transform 200ms' }} />
    </span>
  );
}

function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #f5f4ee' }}>
      <span style={{ fontSize: 13, color: '#44403c' }}>{label}</span>
      {children}
    </div>
  );
}
