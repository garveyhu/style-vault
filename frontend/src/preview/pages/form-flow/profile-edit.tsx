import { ArrowLeft, Bell, Heart, LogOut, Palette, Search, Settings, Star, Upload, User } from 'lucide-react';
...
export default function ProfileEditPagePreview() {
  ...
  const [bio, setBio] = useState('在构建 AI 技能社区 · SkillHub 维护者');
  const [avatar, setAvatar] = useState('user');
  const [gender, setGender] = useState<'male' | 'female' | 'other'>('male');
  const [bioFocus, setBioFocus] = useState(false);

  const SelectedIcon = AVATAR_OPTIONS.find((o) => o.name === avatar)?.Icon ?? User;
  ...
  return (
    <PreviewFrame bg="#f5f7fa">
      <div style={{ maxWidth: 672, margin: '0 auto', fontFamily: 'Inter, system-ui, sans-serif', color: '#111827' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: 8, width: '100%' }}>
              {AVATAR_OPTIONS.map((opt) => {
                const active = opt.name === avatar;
                return (
                  <button key={opt.name} onClick={() => setAvatar(opt.name)}
                    style={{
          ...
          <div>
            ...
            <textarea
              ...
              style={{
                ...
                transition: 'all 200ms', fontFamily: 'inherit', resize: 'none',
              }}
            />
            <div style={{ textAlign: 'right', fontSize: 12, color: '#9ca3af', marginTop: 4 }}>{bio.length}/200</div>
          </div>
        ...
        {/* 危险区 */}
        <section style={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(254,205,211,0.6)', padding: 24 }}>
          <label style={{ display: 'block', fontSize: 14, fontWeight: 700, color: '#be123c', marginBottom: 8 }}>账号</label>
          <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 16px' }}>退出登录后本设备的会话会失效。</p>
          <button style={{
            display: 'inline-flex', alignItems: 'center', gap: 6,
            padding: '8px 16px', borderRadius: 12,
            border: '1px solid #fda4af', background: 'transparent',
            color: '#e11d48', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit',
          }}>
            <LogOut size={14} /> 退出登录
          </button>
        </section>
